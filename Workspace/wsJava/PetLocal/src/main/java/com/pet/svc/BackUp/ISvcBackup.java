...
public interface ISvcBackup{
	
	String sltDetail(Model model, DtoBBS dto);

	String sltList(Model model, DtoBBS dto); // 게시판, 게시글 기준으로 평가 전체 조회, // 게시판, 평점 전체 조회
	//	sltSubject(Model model, DtoBBC dto);
	//	sltWriter(Model model, DtoBBC dto);
	//	sltNewest(Model model, DtoBBC dto);
	//	sltEvl(Model model, DtoBBC dto);
	//	sltClick(Model model, DtoBBC dto);
	//	sltLocation(Model model, DtoBBC dto);
	//	sltBsnTime(Model model, DtoBBC dto);
	//	sltMyEntrprs(Model model, DtoBBC dto);
